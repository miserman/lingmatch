...
test_that('entry order is arbitrary for vector x matrix comparisons', {
  ...
  expect_true(all(abs(sims_ab - sims_ba) < 1e-13))
})
...
test_that('results align with r implementation', {
  ...
  expect_true(all(vapply(seq_len(10), function(i){
    a = rnorm(1000)
    b = rnorm(1000)
    all(abs(lma_simets(a, b) - manual(a, b)) < 1e-13)
  }, TRUE)))
})

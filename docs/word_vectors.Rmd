...
***
# In-sample vectors
The [introduction](index.html#introduction) mostly focused on the representation of documents as vectors within a common document-term space. With that same representation, we can focus on the term vectors instead. 
```{r}
library(splot)
library(lingmatch)
...
Comparing word vectors can provide semantic information. For example, we can see which animals co-occur with different attribute:
...
splot(
  attribute_loadings[, 1] ~ attribute_loadings[, 2], type = 'scatter',
  lines = FALSE, laby = 'smallness', labx = 'bigness',
  add = text(
...
Pretrained spaces like this are available on Open Science Framework ([osf.io/489he](https://osf.io/489he/wiki/home)), and will be downloaded if necessary when you run the functions that depend on them.
...
```{r}
lingmatch(texts, 'pairwise', type = 'lsa')$sim
```

For Word Centroid Distance, a space usually also has stop words removed, but might then be frequency weighted (normalized), mapped to a word2vec space, and have distances measured by Euclidean distance (which is inverted here for similarity):
```{r}
lingmatch(
  texts, 'pairwise', exclude = 'function', weight = 'freq',
  space = 'google', metric = 'euclidean'
)$sim
```

These parameter decisions are often left unexamined, but have some potential to affect the ultimate characterization of the texts. One of `lingmatch`'s goals is to make some of these parameters easy to play with.

## Choosing a space

### Term coverage
One simple thing to look at when choosing a space is its coverage of the terms that appear in your set of texts. You can do this without downloading all spaces by looking at the [common term map](https://osf.io/yauzm), which is what the `select.lsspace` function does when terms are entered as the first argument:
```{R}
# some relatively rare terms for their formatting or narrow uses
terms = c('part-time', 'i/o', "'cause", 'brexit', 'debuffs')

# select.lsspace returns more information about each space,
# as well as the full common term map
spaces = select.lsspace(terms)
...
### Term similarity
Another possible, though more intensive thing to look at might be similarities between words of particular interest. In out initial set of texts, for example, we were looking at animals in relation to attributes, so we might look at those relationships in whatever spaces you've downloaded:
```{r, fig.height=4, fig.width=10, dev='CairoSVG'}
# retrieve names of downloaded spaces
space_names = unique(sub('\\..*$', '', list.files(getOption('lingmatch.lspace.dir'), 'dat')))
...
similarities = do.call(rbind, lapply(space_names, function(name){
  lss = lma_lspace(terms, name)
  if(all(terms %in% rownames(lss))){
    lma_simets(lss, 'canberra')@x
  }else numeric(length(terms) * (length(terms) - 1) / 2)
}))
...
splot(
  smallness ~ bigness, sims_best, type = 'scatter',
  lines = FALSE, add = text(y + .0017 ~ x, labels = terms[-(1:2)])
)
```
...
sims_best_fuz = lma_simets(
  lss_exp[terms,], 'canberra', symmetrical = TRUE
)[-(1:2), c('small', 'big')]

# and see if animal relationships are improved
splot(
  sims_best_fuz[, 'small'] ~ sims_best_fuz[, 'big'], type = 'scatter',
  lines = FALSE, laby = 'smallness', labx = 'bigness',
...
```{r, fig.height=2, fig.width=10, dev='CairoSVG'}
...
top_terms = function(dims, space, nterms = 30){
  ...
  splot(
    terms ~ names(terms), colorby = terms, leg = FALSE, sort = TRUE,
    type = 'bar', laby = FALSE, labx = FALSE, title = paste('dimension',
      if(length(dims) != 1 && sum(dims) == sum(seq_along(dims)))
        paste0(dims[1], '-', dims[length(dims)]) else paste(dims, collapse = ', ')
    )
  )
}
...
```{r, fig.height=2.5, fig.width=10, dev='CairoSVG'}

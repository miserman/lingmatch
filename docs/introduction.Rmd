...
# and so on the words and dtm objects all line up, so words[[i]] corresponds to dtm[i,]
...
term_frequency = sapply(term_weights, function(w)
  lma_weight(matrix(document_frequency, 1), w, FALSE))
...
term_frequency = sapply(doc_weights,function(w)
  lma_weight(sapply(document_frequency, function(i)
    sample(0:i, 5000, TRUE)), w, FALSE, doc.only = TRUE)
)

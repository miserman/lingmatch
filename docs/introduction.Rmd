...
# so one or more of the characters inside the brakets will be replaced with nothing.
...
dtm = matrix(0,length(text),length(unique_words),dimnames=list(c(),unique_words))
dtm[,1:12]
```
...
```{r}
# this will perform what is inside the curly brakets 4 times
# here i keeps track of the loop number, which also lines up with our word list and dtm rows
for(i in 1:4){
...
dimension reduction, and similarity metric.

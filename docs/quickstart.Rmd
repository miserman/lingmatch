...
Continue for more about **[loading text into R](#Loading_your_texts)**, **[processing texts](#Processing_text)**,
and **[measuring similarity](#Measuring_matching)**, or see the [comparisons guide](https://miserman.github.io/lingmatch/#comparisons) for more about defining comparisons.
...
* Give the full path (you can use `/` or `\\` as separators), e.g.,
  ...
  + Mac: `'/user/name/documents/texts.txt'`
* Give a relative path (use `rnormalizePath('example')` to see the full path):
  + with a tilde (see what that starts at with `path.expand('~')`), e.g., `'~/Documents/texts.txt'`
...
## From a delimited plain test file

When tests are in a column of a spreadsheet, stored in a plain text file:
```r
# comma delimited
...
For example, matching between structural features (e.g., number of words and their average length) gives a sense of how
similar the text itself is between texts:
```r

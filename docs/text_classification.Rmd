...
rpart.plot(weight_dtree, extra = 2, mar = c(0, 0, 0, 0))
```
...
splot(
  weight_accuracies ~ weights, type = 'point', title = FALSE,
  leg.title = 'Stop words', laby = 'Overall Accuracy', labx = 'Weight Scheme',
  lpos = 'bottomleft', note = FALSE, sort = TRUE
)
...
Model_Accuracies = do.call(rbind, parLapply(cl, seq_len(100), function(i){
  ...
  # train models
  terms = colMeans(train_dtm[feminine_texts,] != 0) - colMeans(train_dtm[!feminine_texts,] != 0)
  bayes = bernoulli_naive_bayes((train_dtm != 0) * 1, data[ids, 'Recall'], laplace = 1)

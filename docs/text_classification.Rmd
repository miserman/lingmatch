...
that they behaved in either a feminine or masculine manner ([osf.io/ra8zy](https://osf.io/ra8zy/wiki/home)).
...
# remove entries without recall texts
data = data[data$Recall_Text != '',]
```
...
# but be cognizant of information passing between splits (such as if you were to document weight before splitting)
...
term_diffs = colMeans(dtm[feminine_texts,] != 0) - colMeans(dtm[!feminine_texts,] != 0)
...
# words sized by frequency, colored by speaker, and shaded by absolute difference
library(wordcloud)
library(splot)

terms = unlist(lapply(dict, names), use.names = FALSE)
wordcloud(
  terms, sqrt(colSums(dtm[, terms])), scale = c(4, 1),
...
legend(
  'topleft', legend = names(dict), text.col = splot.color(),
  text.font = 2, bty = 'n'
)
```
...
```{r}
library(naivebayes)
weights_nb = bernoulli_naive_bayes((dtm != 0) * 1, data[train_ids, 'Recall'], laplace = 1)
...
but considering conditional differences in terms.

Just for variety, we can also process texts differently to get a different set of features.

```{r}
library(rpart)

# use lma_process to calculate both function word categories and metastatistics
processed = lma_process(data$Recall_Text, dict = lma_dict())

weight_dtree = rpart(
  data[train_ids, 'Recall'] ~ ., processed[train_ids, -1],
  control = rpart.control(cp = .001, minsplit = 2)
)
```
...
Many models also have regularizing version available (that effectively select features),
...
# process texts in the same way, but add tokens.only = TRUE to preserve sequence information
...
weights_neural = keras_model_sequential(list(
  ...
  # this is the final prediction layer, which converts the previous arbitrary layers to a binary classification
  layer_dense(units = 1, activation = 'sigmoid')
))

# then compile the model (this changes the weights_neural object)
compile(
  weights_neural, loss = 'binary_crossentropy',
...
```{r}
# load a set of pre-trained embeddings, with unrecognized terms added with weights of 0
space = lma_lspace(names(tokenized$tokens), 'glove', fill.missing = TRUE)

weights_neural_glove = keras_model_sequential(list(
  # this layer contains the loaded embeddings, which will not be adjusted during training
  layer_embedding(
...
compile(
  weights_neural_glove, loss = 'binary_crossentropy',
  optimizer = 'adam', metrics = 'accuracy'
)
fit(
...
predictions = data.frame(
  ...
  dtree = predict(weight_dtree, processed[-train_ids, -1], 'class'),
  svm = predict(weights_svm, test_dtm),
  neural = classes[(predict(weights_neural, test_sequences) < .5) + 1],
  neural_glove = classes[(predict(weights_neural_glove, test_sequences) < .5) + 1]
)

# when you have multiple classifiers, you can also add an ensemble classifier based on them
# by, for example, taking the most-predicted class
predictions$ensemble = classes[(rowMeans(predictions == classes[1]) < .5) + 1]

# overall accuracies
data.frame(accuracies = colMeans(predictions == data[-train_ids, 'Recall']))
```
...
different weighting schemes on the term-based model:
...
weights = c('pmi', 'ppmi', as.character(outer(
  ...
  c('dflog', 'entropy', 'ppois', 'dpois', 'dfmlog', 'dfmax', 'df', 'idf', 'ridf', 'normal'),
  paste, sep = '-'
)))

# set up an empty matrix for results
weight_accuracies = matrix(
  numeric(length(weights) * 2), ncol = 2,
...
for(exclude in c(TRUE, FALSE)){
  ...
  tdtm = lma_dtm(data[train_ids, 'Recall_Text'], if(exclude) lma_dict() else NULL, dc.min = 5)
  ...
  for(weight in weights){
    ...
    # and take the biggest as the class prediction
    ...
    weight_accuracies[weight, exclude + 1] = mean(wterm_preds == data[train_ids, 'Recall'])
  }
}

splot(
  weight_accuracies ~ weights, type = 'point', title = FALSE, leg.title = 'Stop words',
  laby = 'Overall Accuracy', labx = 'Weight Scheme', note = FALSE, sort = TRUE
)
...
For example, we might compare the performance of the Naïve Bayes, Decision Tree, and Support Vector Machine models.
...
clusterExport(cl, c('classes', 'data', 'processed', 'full_dtm'))
...
Model_Accuracies = do.call(rbind, parLapply(cl, seq_len(100), function(i){
  ...
  # train models
  bayes = bernoulli_naive_bayes((full_dtm[ids,] != 0) * 1, data[ids, 'Recall'], laplace = 1)
  dtree = rpart(
    data[ids, 'Recall'] ~ ., processed[ids, -1],
    control = rpart.control(cp = .001, minsplit = 2)
  )
  svm = train(as.matrix(full_dtm[ids,]), data[ids, 'Recall'], 'svmLinear3')

  # test models
  colMeans(data.frame(
    bayes = predict(bayes, (full_dtm[-ids,] != 0) * 1),
    dtree = predict(dtree, processed[-ids, -1], 'class'),
    svm = predict(svm, full_dtm[-ids,])
  ) == data[-ids, 'Recall'])
}))
stopCluster(cl)

data.frame(Average = c(
  colMeans(Model_Accuracies),
  `bayes > dtree` = mean(Model_Accuracies[, 1] > Model_Accuracies[, 2]),
  `dtree > svm` = mean(Model_Accuracies[, 2] > Model_Accuracies[, 3])
))
splot(Model_Accuracies, title = FALSE, leg.title = 'Model')
```

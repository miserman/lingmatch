...
weights_nb = bernoulli_naive_bayes(
  (dtm != 0) * 1, data[train_ids, 'Recall'], laplace = 1
)
...
rpart.plot(
  weight_dtree, extra = 2, mar = c(0, 0, 0, 0),
  branch.col = "white", split.box.col = "black", split.col = "white",
  nn.box.col = "black", nn.col = "white"
)
```
...
weights = c('pmi', 'ppmi', as.character(outer(
  ...
  c('dflog', 'entropy', 'ppois', 'dpois', 'dfmlog', 'dfmax', 'df', 'idf',
    'ridf', 'normal'),
  paste, sep = '-'
)))
...
for(exclude in c(TRUE, FALSE)){
  ...
  tdtm = lma_dtm(
    data[train_ids, 'Recall_Text'], if(exclude) lma_dict() else NULL, dc.min = 5
  )
...
splot(
  ...
  lpos = 'bottomleft', note = FALSE, sort = TRUE, dark = TRUE
)
...
Model_Accuracies = do.call(rbind, parLapply(cl, seq_len(100), function(i){
  ...
  bayes = bernoulli_naive_bayes(
    (train_dtm != 0) * 1, data[ids, 'Recall'], laplace = 1
  )

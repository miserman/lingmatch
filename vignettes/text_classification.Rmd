...
```{r, fig.height=5, fig.alt="a wordcloud showing the most feminine- and masculine-associated terms"}
...
```{r, fig.height=4.5, fig.alt="a decision tree that splits on terms to predict feminine or masculine text"}
library(rpart)
library(rpart.plot)
...
(see [keras3.posit.co](https://keras3.posit.co/)).
...
# specify your layers
weights_neural <- keras_model_sequential(layers = list(
...
weights_neural_glove <- keras_model_sequential(layers = list(
  # this layer contains the loaded embeddings, which will not be adjusted
  # during training
  layer_embedding(
    input_dim = nrow(space) + 1, output_dim = ncol(space),
    trainable = FALSE, weights = list(rbind(numeric(ncol(space)), space))
  ),
...
```{r, fig.height=5, fig.alt="a plot comparing accuracy with different weighting schemes, and keeping vs excluding function words"}
...
```{r, fig.height=4, fig.alt="a plot showing the accuracy distribution of different models with different train-test splits"}
library(parallel)

...
top_suggested <- meta$suggested$furniture[1:10]
nodes <- data.frame(
...
visNetwork(
  nodes, within(edges[edges$value > .1 & edges$value < 1, ], value <- (value * 10)^4)
) |>
  visEdges("title", smooth = FALSE, color = list(opacity = .6)) |>
  visLegend(width = .07) |>
  visPhysics(
    "forceAtlas2Based",
    forceAtlas2Based = list(centralGravity = .03, avoidOverlap = .5), timestep = 1e-4
  ) |>
  visLayout(randomSeed = 2080) |>
  visInteraction(
